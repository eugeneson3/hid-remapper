...
bool replace_queued_absolute_report(uint8_t report_id) {
    ...
    // Keep the newest authoritative keyboard state when the USB IN endpoint
    // is temporarily back-pressured. Replacing an older absolute report is
    // safer than dropping a key-up or key-down transition on queue overflow.
